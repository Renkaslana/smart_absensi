...
import { faceAPI, canvasToBase64 } from '@/lib/api';
...
export default function AdminFaceCheckPage() {
  ...
  const captureAndScan = async () => {
    ...
    try {
      ...
      // expected: { data: { name, nim, confidence, status } }
      const payload = res.data;
      if (payload?.name) {
        setResult({ name: payload.name, nim: payload.nim, confidence: payload.confidence });
      } else if (payload?.detail) {
        setResult({ name: 'Tidak Dikenal', message: payload.detail });
      } else {
        setResult({ name: 'Tidak Dikenal', message: 'Tidak ada kecocokan.' });
      }
    } catch (err: any) {
          ...
          {result && (
            <div className="space-y-4">
              ...
              <div className="flex space-x-2">
                <button onClick={captureAndScan} disabled={scanning} className="flex-1 inline-flex items-center justify-center px-4 py-2 bg-green-600 text-white rounded-md">Scan Lagi</button>
                <button onClick={() => { setResult(null); }} className="flex-1 inline-flex items-center justify-center px-4 py-2 bg-gray-100 text-gray-700 rounded-md">Reset</button>
              </div>
