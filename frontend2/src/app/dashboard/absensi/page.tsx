...
export default function AbsensiPage() {
  ...
  const startLivenessDetection = useCallback(async () => {
    ...
    try {
      ...
      // Check if MediaPipe initialized successfully
      const isMediaPipeReady = mediaPipeService['isInitialized'];
      
      if (!isMediaPipeReady) {
        // Skip liveness detection if MediaPipe failed
        console.warn('⚠️ MediaPipe not available, skipping liveness detection');
        speak('Memproses pengenalan wajah');
    ...
    try {
      ...
      if (scanResponse.data.recognized) {
        ...
        if (submitResponse.data?.already_submitted) {
          ...
          setSuccessNotification(`Absensi berhasil! Selamat datang, ${recognizedData.name}`);
          ...
          <AnimatePresence>
            {successNotification && (
              <motion.div
                ...
                animate={{ opacity: 1, y: 0 }}
                exit={{ opacity: 0, y: 20 }}
                className="absolute bottom-0 left-0 right-0 bg-green-500 text-white px-6 py-4 text-center font-medium"
              >
                {successNotification}
              </motion.div>
            )}
