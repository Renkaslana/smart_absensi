...
class MediaPipeService {
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      this.faceDetection = new FaceDetection({
        locateFile: (file) => {
          // Use local files from public directory (copied by predev script)
          console.log('📦 [MediaPipe] Loading face_detection file:', file);
      ...
      this.faceMesh = new FaceMesh({
        locateFile: (file) => {
          // Use local files from public directory (copied by predev script)
          console.log('📦 [MediaPipe] Loading face_mesh file:', file);
      ...
      console.warn('⚠️ [MediaPipe] Continuing without liveness detection');
      // Don't throw error, just log warning and continue without MediaPipe
    }
  }
  ...
  async detectLiveness(imageElement: HTMLVideoElement): Promise<LivenessResult> {
    ...
    // If MediaPipe failed to initialize, skip liveness detection
    if (!this.faceMesh || !this.isInitialized) {
      console.warn('⚠️ [MediaPipe] Liveness detection skipped - MediaPipe not available');
      return {
        isLive: true, // Auto-pass if MediaPipe not available
        blinkDetected: false,
